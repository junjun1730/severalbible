...
void main() {
  ...
  group('createPrayerNote', () {
    test('should return Right(PrayerNote) when datasource succeeds', () async {
      // Arrange
      when(() => mockDataSource.createPrayerNote(
      ...
      verify(() => mockDataSource.createPrayerNote(
            userId: testUserId,
            content: 'Today I reflected on God\'s grace and mercy.',
            scriptureId: testScriptureId,
          )).called(1);
    ...
    test('should return Left(ValidationFailure) when content is empty',
        () async {
      // Act
      final result = await repository.createPrayerNote(
        userId: testUserId,
      ...
      verifyNever(() => mockDataSource.createPrayerNote(
            userId: any(named: 'userId'),
            content: any(named: 'content'),
            scriptureId: any(named: 'scriptureId'),
          ));
  ...
  group('getPrayerNotes', () {
    test('should return Right(List<PrayerNote>) when datasource succeeds',
        () async {
      // Arrange
      when(() => mockDataSource.getPrayerNotes(
            userId: any(named: 'userId'),
      ...
      verify(() => mockDataSource.getPrayerNotes(
            userId: testUserId,
            startDate: null,
            endDate: null,
          )).called(1);
  ...
  group('getPrayerNotesByDate', () {
    test('should return notes for specific date', () async {
      // Arrange
      when(() => mockDataSource.getPrayerNotesByDate(
            userId: any(named: 'userId'),
            date: any(named: 'date'),
          )).thenAnswer((_) async => [samplePrayerNoteJson]);
      ...
      verify(() => mockDataSource.getPrayerNotesByDate(
            userId: testUserId,
            date: testDateTime,
          )).called(1);
    });
  ...
  group('updatePrayerNote', () {
    ...
    test('should return Right(PrayerNote) with updated content', () async {
      // Arrange
      when(() => mockDataSource.updatePrayerNote(
      ...
      verify(() => mockDataSource.updatePrayerNote(
            noteId: testNoteId,
            content: 'Updated meditation content',
          )).called(1);
    });
  ...
  group('deletePrayerNote', () {
    test('should return Right(void) when deletion succeeds', () async {
      // Arrange
      when(() => mockDataSource.deletePrayerNote(
            noteId: any(named: 'noteId'),
          )).thenAnswer((_) async => {});
      ...
      // Assert
      expect(result.isRight(), true);
      verify(() => mockDataSource.deletePrayerNote(noteId: testNoteId))
          .called(1);
    });
    ...
    test('should return Left(ServerFailure) when note not found', () async {
      ...
      // Act
      final result =
          await repository.deletePrayerNote(noteId: 'non-existent-id');
  ...
  group('isDateAccessible', () {
    test('should return true for today (all tiers)', () async {
      ...
      when(() => mockDataSource.isDateAccessible(
            userId: any(named: 'userId'),
            date: any(named: 'date'),
