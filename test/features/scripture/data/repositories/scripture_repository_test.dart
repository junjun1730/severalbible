...
void main() {
  ...
  group('getRandomScripture', () {
    test('should return Right(List<Scripture>) when datasource succeeds',
        () async {
      // Arrange
      when(() => mockDataSource.getRandomScripture(any()))
    ...
    test('should return Left(ServerFailure) when datasource throws exception',
        () async {
      // Arrange
      when(() => mockDataSource.getRandomScripture(any()))
    ...
    test('should return Right with empty list when no scriptures available',
        () async {
      // Arrange
      when(() => mockDataSource.getRandomScripture(any()))
          .thenAnswer((_) async => []);
  ...
  group('getDailyScriptures', () {
    test('should return Right(List<Scripture>) when datasource succeeds',
        () async {
      // Arrange
      when(() => mockDataSource.getDailyScriptures(
      ...
      verify(() => mockDataSource.getDailyScriptures(
            userId: testUserId,
            count: 3,
          )).called(1);
    });

    test('should return up to requested count from getDailyScriptures',
        () async {
      // Arrange
      when(() => mockDataSource.getDailyScriptures(
            userId: any(named: 'userId'),
  ...
  group('getPremiumScriptures', () {
    test('should return only premium scriptures when datasource succeeds',
        () async {
      // Arrange
      when(() => mockDataSource.getPremiumScriptures(
            userId: any(named: 'userId'),
            count: any(named: 'count'),
      ...
      verify(() => mockDataSource.getPremiumScriptures(
            userId: testUserId,
            count: 3,
          )).called(1);
    });

    test('should return up to requested count from getPremiumScriptures',
        () async {
      // Arrange
      when(() => mockDataSource.getPremiumScriptures(
            userId: any(named: 'userId'),
  ...
  group('recordScriptureView', () {
    test('should return Right(void) when recording succeeds', () async {
      // Arrange
      when(() => mockDataSource.recordScriptureView(
            userId: any(named: 'userId'),
      ...
      // Assert
      expect(result.isRight(), true);
      verify(() => mockDataSource.recordScriptureView(
            userId: testUserId,
            scriptureId: testScriptureId,
          )).called(1);
    });
  ...
  group('getScriptureHistory', () {
    test('should return scriptures for given date when history exists',
        () async {
      // Arrange
      when(() => mockDataSource.getScriptureHistory(
            userId: any(named: 'userId'),
            date: any(named: 'date'),
          )).thenAnswer((_) async => [sampleScriptureJson]);
      ...
      verify(() => mockDataSource.getScriptureHistory(
            userId: testUserId,
            date: testDateTime,
          )).called(1);
    });
