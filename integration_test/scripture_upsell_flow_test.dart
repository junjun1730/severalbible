...
void main() {
  ...
  Widget createSubject({required UserTier tier, required List<Scripture> scriptures}) {
    return ProviderScope(
      overrides: [
        currentUserTierProvider.overrideWith((ref) => Stream.value(tier)),
        dailyScripturesProvider.overrideWith((ref) => AsyncValue.data(scriptures)),
  ...
  testWidgets('shows upsell dialog when member tries to view 4th scripture', (tester) async {
    ...
    await tester.pumpWidget(createSubject(
      ...
      scriptures: testScriptures.take(3).toList(), // Limit to 3 for test scenario
    ));
